...
- Chunk大小与重叠
- 层级索引：文档级 -> 章节级 -> 段落级。
- 保留结构信息：把标题路径（H1/H2/H3）/章节名/页面号/代码文件路径写进metadata。
- 表格与代码专门处理：表格可做“行列语义化”或生成“表格摘要+关键字段”；代码按函数/类切并附上文件路径、符号名。

#### Embedding与向量表示优化

- 领域适配embedding：通用模型在垂直领域可能会差一点；优先选领域强的embedding或做轻量微调/对比学习。
- 多向量/多视图索引：
    - 同一chunk生成
    - 查询时分别召回再融合，能提升“短查询/含术语查询”的召回率。
- 去噪与规范化：清理模版页眉页脚、重复免责声明、导航菜单；统一单位/时间格式；减少embedding被噪声主导。
- 去重（dedup）：对相似chunk做MinHash/SimHash或embedding聚类去重，避免一堆重复片段占据topK。
