...
**物理含义**：
基于上一时刻的最佳估计状态 $\hat{x}_{t-1|t-1}$，通过系统动力学模型 $F_t$（如牛顿运动定律）推演到当前时刻。如果有外部控制输入 $u_t$（如刹车、加速），也通过 $B_t$ 叠加进去。
- $\hat{x}_{t|t-1}$：先验估计（A Priori Estimate），即在还没看观测值之前的猜测。

#### 2.2 协方差预测（Covariance Prediction）
...
**物理含义**：
预测不仅推演了状态，也推演了**不确定性**（误差协方差）。
1. $F_t P_{t-1|t-1} F_t^T$：误差随系统动态传播。比如速度有误差，时间久了位置误差会越来越大。
2. $+ Q_t$：加上过程噪声协方差。因为物理模型不仅是不完美的（有 $w_t$），而且随时间推移，不确定性会累积，所以不确定性 $P$ 会变大。

---

### 3. **更新阶段（Update）**

#### 3.1 卡尔曼增益计算（Kalman Gain）
...
**物理含义**：
卡尔曼增益 $K_t$ 决定了我们是更相信**预测值**还是**观测值**。
可以理解为权重比率：$K_t \approx \frac{\text{预测误差}}{\text{预测误差} + \text{测量误差}}$
- 如果测量噪声 $R_t$ 很大（传感器很不准），$K_t$ 会很小 $\to$ 更多保留预测值。
- 如果预测误差 $P_{t|t-1}$ 很大（模型很不准），$K_t$ 会接近 1（或 $H^{-1}$） $\to$ 更多去相信观测值 $z_t$。

#### 3.2 状态更新（State Update）
...
**物理含义**：
后验估计 = 先验预测 + 修正项。
- $(z_t - H_t \hat{x}_{t|t-1})$：**新息（Innovation）**或残差。即“实际看到的”减去“预计能看到的”。
- $K_t$：将这个“惊奇”的程度（残差）按比例修正到状态上。

#### 3.3 协方差更新（Covariance Update）
...
**物理含义**：
更新后的不确定性 $P_{t|t}$。
因为结合了观测数据，信息量增加了，所以不确定性通常会**减小**（$(I - K_t H_t)$ 通常小于 1）。我们比单纯预测时更确信当前的状态了。
